...
const CoordinatesReportForm = () => {
  ...
  const handleSubmitForm = async (values, { setSubmitting }) => {
    ...
    try {
      const data = await axiosApiInstance.post('/admin/report/points', values);
      ...
      // worksheet.getCell('C2').value = 'التقرير المجمع لبيانات الفحص';
      ...
      // worksheet.getCell(
      //   'C6'
      // ).value = `لمحصول ${crop[0].name_ar} للموسم المنتهى فى ${values.season}`;
...
export default CoordinatesReportForm;
