...
function FilterComponents({
  ...
  function handleGovChange(govIds) {
    ...
    //   });
    axiosApiInstance
      .post(`/admin/location/getcenters`, {
        governorates: govIds,
      })
      .then((data) => {
        setCenter(data);
      ...
      axiosApiInstance
        ...
        .then((response) => {
          ...
          if (response.data.length > 0) {
            ...
            // only to add selectedToMap property to be used to helight when row get sleected/clickedOn to move to searchMapTable
            const newData = response.data.map((request) => {
              return { ...request, selectedToMap: false };
            });
            // console.log(newData);
            setRenderPolygons(newData);
            setShowingNow([]);
            setWhatsFormShowingNow([]);
            setseachedLand([]);
            setSubmitting(false);
