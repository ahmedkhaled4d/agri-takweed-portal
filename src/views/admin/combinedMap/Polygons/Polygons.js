...
export const Polygons = ({
  ...
  useEffect(() => {
    ...
    // console.log(googleMapsPolygonInstances);
    mapUtils.fitBounds(googleMapsPolygonInstances, map);
    // console.log(renderedpolygonsWithPolygonsInstancs);
  ...
  useEffect(() => {
    ...
    const filteredPolygons = renderedpolygonsWithPolygonsInstancs.filter(
      (el) => {
        ...
        else if (el.farmPhone === seachedLand.farmPhone) return el;
      }
    );
    // console.log({ filteredPolygons });
    const googleMapsPolygonInstances = filteredPolygons.map((el) => {
      return { polygon: el.polygon, marker: el.marker.marker };
    });
    ...
    //fit the bounds
    mapUtils.fitBounds(googleMapsPolygonsOnlyInstances, map);
