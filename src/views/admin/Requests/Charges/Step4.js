...
function SubmitForm({
  ...
  function getData() {
    if (reqCode) {
      axiosApiInstance
        .get(`/admin/traceability/${reqCode}/dist`)
        .then((response) => {
  ...
  useEffect(() => {
    axiosApiInstance.get("/admin/hub?type=EXPORT").then((res) => {
      //  console.log('distributers',res);
      //  console.log(res.data.data);
      setExportersData(res.data.data);
    });
  }, []);
  useEffect(() => {
    ...
    if (selectedDist) {
      ...
      hubToShow = distributersData.find((el) => {
        return el.hubId === selectedDist;
      });
    }
  ...
  const handleSubmitForm = (values, { setSubmitting, resetForm }) => {
    if (!errorExists) {
      ...
      axiosApiInstance
        .post(
          `/admin/traceability/${reqCode}/${selectedDist}/send/${values.exporter}`,
          data
        )
        .then((response) => {
          setSubmitting(false);
  ...
  return (
    <>
      <Formik
        ...
        validate={(values) => {
          const errors = {};
          if (!values.distributer) {
            errors.distributer = "مطلوب";
          }
          if (!values.exporter) {
            errors.exporter = "مطلوب";
          }
          return errors;
        }}
          ...
          <Form className={styles.ReportsTableButtons} onSubmit={handleSubmit}>
            ...
            >
              ...
              <FormGroup className="mb-0">
                ...
                <span className="text-danger">
                  {errors.exporter && touched.exporter && errors.exporter}
                </span>
              </FormGroup>
              <Button
